...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    ...
    const SCRIPT_URL = import.meta.env.VITE_RIDER_SCRIPT_URL;

    if (!SCRIPT_URL) {
      throw new Error("VITE_RIDER_SCRIPT_URL is not defined in environment variables");
    }
    ...
    console.log(
      "Rider Registration Data submitted successfully:",
      response.data,
    );
    ...
    return {
      ...
      title: "สมัครสำเร็จ!",
      message:
        "ข้อมูลของคุณถูกส่งเข้าระบบเรียบร้อยแล้ว ทีมงานจะทำการติดต่อกลับโดยเร็วที่สุด",
    };
  } catch (error: any) {
          ...
          <Form method="post" className="space-y-6">
            ...
            <div className="flex items-start mt-4">
              ...
              <div className="ml-3 text-sm">
                ...
                <p className="text-gray-500">
                  ข้าพเจ้ายืนยันว่าข้อมูลข้างต้นเป็นความจริงทุกประการและยินยอมให้เก็บรวบรวมข้อมูล
                </p>
              </div>
            </div>
            ...
            <div className="pt-4">
              ...
              >
                ...
                <span
                  className={`relative flex items-center gap-2 ${isSubmitting ? "hidden" : "flex"}`}
                >
                ...
                <span
                  className={`relative flex items-center gap-2 ${isSubmitting ? "flex" : "hidden"}`}
                >
      ...
      {modal.show && (
        <div className="fixed inset-0 z-50 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity duration-300">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center transform transition-transform duration-300 scale-100">
            <div
              className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4 ${modal.type === "success" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
            >
              {modal.type === "success" ? (
